			...
			<el-form ref="form" :rules="form_rules" :model="form" size="mini" label-width="15%">
				<el-tabs type="border-card" v-model="page_tag">
					...
					<el-tab-pane :label="page_info.title" name="1">
						<el-form-item :disabled="page_info.is_admin_edit" label="类型" prop="set_type">
							<el-radio-group v-model="form.set_type">
								<el-radio v-for="(val,key) in page_info.list_type" :key="key" :label="key">
									{{val}}
								</el-radio>
							</el-radio-group>
						</el-form-item>
						<el-form-item label="名称" prop="set_name">
							<el-input class="x-input-60" v-model="form.set_name"></el-input>
						</el-form-item>
						<el-form-item label="属性" prop="set_item_data">
							<JsonToItem v-bind:value.sync="form.set_item_data" :show="{show_list:form.set_type==1}"></JsonToItem>
						</el-form-item>
...
<script>
	import JsonToItem from '@/components/JsonToItem'
	...
	export default {
		...
		mounted() {
			...
			//
			this.g_page.funcSetApiUrlEdit('/xadmin/website/website__option__edit')
			this.g_page.funcSetApiUrlSave('/xadmin/website/website__option__edit/save')
			let page = this
			//验证
			this.form_rules = {
				__test: [{
