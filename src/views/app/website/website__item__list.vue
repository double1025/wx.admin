<template>
	<el-container>
		<el-header class='x-header'>
			...
			<el-button size="mini" type="success" icon="el-icon-edit" @click="g_page.funcRedirectEdit(`website__item__edit?pid=${form_q.pid}`)">
				添加
			</el-button>
				...
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="编号">
						<template slot-scope="data">
							<div v-html="data.row.sdata_id"></div>
						</template>
					</el-table-column>
					<el-table-column v-for="(item,index) in page_info.item_data" :label="item.label" :key="index" v-if="item.show_list">
						<template slot-scope="data">
							<div v-if="item.type!='upload_img'" v-html="data.row.json[item.field_key]"></div>
							<div v-if="item.type=='upload_img'">
								<CompImg :img_root="page_info.static_file_root" :img_val="data.row.json[item.field_key]"></CompImg>
							</div>
						</template>
					</el-table-column>
					<el-table-column label="排序">
						<template slot-scope="data">
							<div v-html="data.row.sdata_order"></div>
						</template>
					</el-table-column>
					<el-table-column fixed="right" label="操作">
						<template slot-scope="data">
							<el-button type="text" size="mini" @click="g_page.funcRedirectEdit(`website__item__edit?pid=${form_q.pid}`,data.row)">
								编辑
							</el-button>
						</template>
					</el-table-column>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/website/website__item__list')
			this.g_page.funcSetApiUrlDel('/xadmin/website/website__item__list/del')
			//页码
