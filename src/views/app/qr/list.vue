			...
			<div class="table-wrap">
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="回复类型">
						<template slot-scope="data">
							{{data.row.reply_type_str}}
						</template>
					</el-table-column>
					<el-table-column fixed="right" label="操作">
						<template slot-scope="data">
							<el-button type="primary" size="mini" @click="funcEdit(data.row)">查看</el-button>
						</template>
					</el-table-column>
				</el-table>
				<a target="_blank"></a>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/qr/qr__list')
			this.g_page.funcSetApiUrlDel('/xadmin/qr/qr__list/del')
			//页码
		...
		methods: {
			funcEdit(row) {
				...
				this.g_cc.func_redirect('/qr/edit', {
					id: id,
					callback_url: '/qr/list'
				})
			}
			////////////////////
		}
