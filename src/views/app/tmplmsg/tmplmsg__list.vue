<template>
	<el-container>
				...
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="编号">
						<template slot-scope="data">
							<div v-html="data.row.wxtmplmsg_id"></div>
						</template>
					</el-table-column>
					<el-table-column label="名称">
						<template slot-scope="data">
							<div v-html="data.row.wxtmplmsg_name"></div>
						</template>
					</el-table-column>
					<el-table-column label="模板ID">
						<template slot-scope="data">
							{{data.row.wxtmplmsg_tmpl_id}}
						</template>
					</el-table-column>
					<el-table-column label="创建时间">
						<template slot-scope="data">
							{{data.row.wxtmplmsg_create_date}}
						</template>
					</el-table-column>
					<el-table-column fixed="right" label="操作">
						<template slot-scope="data">
							<el-button type="primary" size="mini" @click="funcEdit(data.row.id)">编辑</el-button>
							<el-button size="mini" @click="g_page.funcRedirectEdit(`tmplmsg__send?pid=${data.row.id}`)">
								发送
							</el-button>
						</template>
					</el-table-column>
		...
		<!---->
		<!-- 编辑控件 -->
		<TmplmsgEdit ref="item_edit" id_src="" :funcSaveAfter="funcEditItem_success"></TmplmsgEdit>
		<!---->
		<!---->
		<!-- 弹框 -->
...
<script>
	...
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/tmplmsg/tmplmsg__list')
			this.g_page.funcSetApiUrlDel('/xadmin/tmplmsg/tmplmsg__list/del')
			//页码
		...
		methods: {
			funcEditItem_success(e) {
				console.log('funcEditItem_success', e)
				this.g_page.funcGetList()
			},
			funcEdit(id) {
