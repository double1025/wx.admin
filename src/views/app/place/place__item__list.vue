<template>
	<el-container>
		<el-header class='x-header'>
			...
			<el-button size="mini" type="success" icon="el-icon-edit" @click="g_page.funcRedirectEdit(`place__item__edit`)">
				添加
			</el-button>
		...
		<el-main class='x-main'>
			<el-form :inline="true" :model="form_q" size="mini">
				...
				<el-form-item label="分类" prop="compType">
					<el-select filterable v-model="form_q.compType">
						<el-option
				...
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="编号">
						<template slot-scope="data">
							<div v-html="data.row.pitem_id"></div>
						</template>
					</el-table-column>
					...
					<el-table-column label="名称">
						<template slot-scope="data">
							<div v-html="data.row.pitem_name"></div>
						</template>
					</el-table-column>
					<el-table-column label="地址">
						<template slot-scope="data">
							{{ data.row.pitem_province }}
							{{ data.row.pitem_city }}
							{{ data.row.pitem_district }}
							{{ data.row.pitem_address }}
						</template>
					</el-table-column>
					<el-table-column label="打卡数">
						<template slot-scope="data">
							{{ data.row.pitem_good_count }}
						</template>
					</el-table-column>
					<!--					<el-table-column label="收藏数">-->
					<!--						<template slot-scope="data">-->
					<!--							{{ data.row.pitem_collect_count }}-->
					<!--						</template>-->
					<!--					</el-table-column>-->
					<el-table-column label="状态">
						<template slot-scope="data">
							<div v-if="data.row.pitem_hide==0">显示</div>
							<div v-if="data.row.pitem_hide==1">隐藏</div>
						</template>
					</el-table-column>
					<el-table-column label="排序">
						<template slot-scope="data">
							{{ data.row.pitem_order }}
						</template>
					</el-table-column>
					<el-table-column fixed="right" label="操作" width="100">
						<template slot-scope="data">
							<el-button type="text" size="mini" @click="g_page.funcRedirectEdit('place__item__edit',data.row)">
								编辑
							</el-button>
						</template>
					</el-table-column>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/place/place__item__list')
			this.g_page.funcSetApiUrlDel('/xadmin/place/place__item__list/del')
			//页码
