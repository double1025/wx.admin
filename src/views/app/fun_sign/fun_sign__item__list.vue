<template>
	<el-container>
		<el-header class='x-header'>
			...
			<el-button size="mini" type="success" icon="el-icon-edit" @click="g_page.funcRedirectEdit(`fun_sign__item__edit`)">
				添加
			</el-button>
				...
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="有效时间">
						<template slot-scope="data">
							...
							<div v-html="data.row.abc_date_01"></div>
							<b style="color:red;" v-if="data.row.is_expire">已过期</b>
						</template>
					</el-table-column>
					...
					<el-table-column fixed="right" label="操作">
						<template slot-scope="data">
							<el-button type="primary" size="mini" @click="g_page.funcRedirectEdit('fun_sign__item__edit',data.row)">
								编辑
							</el-button>
							<el-button size="mini" @click="g_page.funcRedirectEdit(`fun__item__list?id_src=${data.row.id}`)">
								签到记录
							</el-button>
						</template>
					</el-table-column>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/fun_sign/fun_sign__item__list')
			this.g_page.funcSetApiUrlDel('/xadmin/fun_sign/fun_sign__item__list/del')
			//页码
