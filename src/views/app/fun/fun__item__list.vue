<template>
	<el-container>
		<el-header class='x-header'>
			...
			<el-button size="mini" type="success" icon="el-icon-edit" @click="g_page.funcRedirectEdit(`fun__item__add`)">
				加积分
			</el-button>
			<ExprotExcel api_url="/xadmin/fun/fun__item__list/export" :api_data="form_q"></ExprotExcel>
			...
			<div class="table-wrap">
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="应用">
						<template slot-scope="data">
							<div v-html="data.row.funrec_app_id"></div>
						</template>
					</el-table-column>
					<el-table-column label="数值">
						<template slot-scope="data">
							<div v-html="data.row.funrec_num"></div>
						</template>
					</el-table-column>
					<el-table-column label="描述">
						<template slot-scope="data">
							<div v-html="data.row.funrec_desc"></div>
						</template>
					</el-table-column>
					<el-table-column label="操作时间">
						<template slot-scope="data">
							<div v-html="data.row.funrec_create_date"></div>
						</template>
					</el-table-column>
					<el-table-column label="过期时间">
						<template slot-scope="data">
							<div v-if="data.row.funrec_num>0">
								{{data.row.funrec_expire_date}}
								<span style="color:#ffd2c3;" v-html="">[{{data.row.funrec_num_left}}]</span>
							</div>
						</template>
					</el-table-column>
				</el-table>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/fun/fun__item__list')
			this.g_page.funcSetApiUrlDel('/xadmin/fun/fun__item__list/del')
			//页码
