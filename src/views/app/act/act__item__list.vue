<template>
	<el-container>
		<el-header class='x-header'>
			...
			<el-button size="mini" type="success" icon="el-icon-edit" @click="g_page.funcRedirectEdit('act__item__edit')">
				添加
			</el-button>
			<el-button size="mini" type="danger" icon="el-icon-delete" @click="g_page.funcDel()">
				删除
			</el-button>
			...
			<div class="table-wrap">
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="有效期">
						<template slot-scope="data">
							<div>{{ data.row.aset_begin_date }}</div>
							<div>{{ data.row.aset_end_date }}</div>
							<div style="color:red;" v-if="data.row.errmsg_date!=''">({{data.row.errmsg_date}})</div>
						</template>
					</el-table-column>
					<el-table-column label="状态">
						<template slot-scope="data">
							<div v-html="data.row.__status"></div>
						</template>
					</el-table-column>
					<el-table-column fixed="right" label="操作">
						<template slot-scope="data">
							<el-button type="text" size="mini" @click="g_page.funcRedirectEdit('act__item__edit',data.row)">
								编辑
							</el-button>
							<el-button type="text" size="mini" @click="g_page.funcRedirectEdit(`/prize/prize__set__list?app_id=act&pid=${data.row.aset_uix}`)">
								奖品配置
							</el-button>
							<el-button type="text" size="mini" @click="g_page.funcRedirectEdit(`act__launch__list?pid=${data.row.id}`)">
								数据
							</el-button>
						</template>
					</el-table-column>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/act/act__item__list')
			this.g_page.funcSetApiUrlDel('/xadmin/act/act__item__list/del')
			//页码
