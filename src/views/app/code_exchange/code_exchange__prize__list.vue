<template>
	<el-container>
			...
			<div class="table-wrap">
				<el-table stripe ref="multipleTable" tooltip-effect="dark" @selection-change="g_page.funcTableSelectionChange"
					...
					<el-table-column label="排序">
						<template slot-scope="data">
							<div>{{data.row.pset_order}}</div>
						</template>
					</el-table-column>
					<el-table-column fixed="right" label="操作">
						<template slot-scope="data">
							<el-button type="primary" size="mini" @click="funcEdit(data.row.id)">编辑</el-button>
							<el-button size="mini" @click="g_page.funcRedirectEdit(`code_exchange__code__list?pid=${data.row.id}`)">
								兑换码
							</el-button>
						</template>
					</el-table-column>
		...
		<!---->
		<!-- 编辑控件 -->
		<PrizeSetEdit :arr_hide="{pschou_win_rate:1,stock:1}" ref="item_edit" :id_src="form_q.pid" :app_id="form_q.app_id" :funcSaveAfter="funcEditItem_success"></PrizeSetEdit>
		<!---->
		<!---->
		<!-- 弹框 -->
...
<script>
	...
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/code_exchange/code_exchange__prize__list')
			this.g_page.funcSetApiUrlDel('/xadmin/code_exchange/code_exchange__prize__list/del')
			//页码
