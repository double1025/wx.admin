<template>
	<el-container>
		<el-header class='x-header'>
			...
			<el-button size="mini" type="success" icon="el-icon-edit" @click="g_page.funcRedirectEdit(`app_fengce__msg__edit?model=${form_q.model}&type=${form_q.type}`)">
				添加
			</el-button>
				...
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="编号">
						<template slot-scope="data">
							<div v-html="data.row.fmsg_id"></div>
						</template>
					</el-table-column>
					<el-table-column label="分类" v-if="form_q.model=='rule'">
						<template slot-scope="data">
					...
					<el-table-column label="模块">
						<template slot-scope="data">
							<div v-html="data.row.model_name"></div>
						</template>
					</el-table-column>
					<el-table-column label="标题">
						<template slot-scope="data">
							<div v-html="data.row.fmsg_title"></div>
						</template>
					</el-table-column>
					<el-table-column label="发布时间">
						<template slot-scope="data">
							<div v-html="data.row.fmsg_post_date"></div>
						</template>
					</el-table-column>
					<el-table-column label="来源">
						<template slot-scope="data">
							<div v-html="data.row.fmsg_author"></div>
						</template>
					</el-table-column>
					<el-table-column label="收藏数">
						<template slot-scope="data">
							<div v-html="data.row.fmsg_collect"></div>
						</template>
					</el-table-column>
					<el-table-column label="浏览数">
						<template slot-scope="data">
							<div v-html="data.row.fmsg_pv"></div>
						</template>
					</el-table-column>
					<el-table-column label="隐藏">
						<template slot-scope="data">
							<div style="color:red;" v-if="data.row.fmsg_hide==1">是</div>
						</template>
					</el-table-column>
					<el-table-column label="排序">
						<template slot-scope="data">
							<div v-html="data.row.fmsg_order"></div>
						</template>
					</el-table-column>
					<el-table-column fixed="right" label="操作">
						<template slot-scope="data">
							<el-button type="text" size="mini" @click="g_page.funcRedirectEdit(`app_fengce__msg__edit?model=${form_q.model}`,data.row)">
								编辑
							</el-button>
						</template>
					</el-table-column>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/app_fengce/app_fengce__msg__list')
			this.g_page.funcSetApiUrlDel('/xadmin/app_fengce/app_fengce__msg__list/del')
			//页码
