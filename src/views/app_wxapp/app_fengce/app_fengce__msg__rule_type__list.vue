<template>
	<el-container>
		<el-header class='x-header'>
			...
			<el-button size="mini" type="success" icon="el-icon-edit" @click="g_page.funcRedirectEdit(`app_fengce__msg__rule_type__edit`)">
				添加
			</el-button>
			...
			<div class="table-wrap">
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="图标">
						<template slot-scope="data">
							<CompImg :img_root="page_info.static_file_root" :img_val="data.row.abc_str_01"></CompImg>
						</template>
					</el-table-column>
					...
					<el-table-column fixed="right" label="操作">
						<template slot-scope="data">
							<el-button type="text" size="mini" @click="g_page.funcRedirectEdit('app_fengce__msg__rule_type__edit',data.row)">
								编辑
							</el-button>
							<el-button type="text" size="mini" @click="g_page.funcRedirectEdit(`app_fengce__msg__rule__list?type=${data.row.id}`)">
								文章列表
							</el-button>
						</template>
					</el-table-column>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/app_fengce/app_fengce__msg__rule_type__list')
			this.g_page.funcSetApiUrlDel('/xadmin/app_fengce/app_fengce__msg__rule_type__list/del')
			//页码
