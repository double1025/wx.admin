		...
		<el-main class='x-main'>
			...
			<div class="table-wrap">
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="导师名字">
						<template slot-scope="data">
							<div v-html="data.row.lteacher_name"></div>
						</template>
					</el-table-column>
					<el-table-column label="导师标签语">
						<template slot-scope="data">
							{{data.row.lteacher_tag}}
						</template>
					</el-table-column>
					<el-table-column label="排序">
						<template slot-scope="data">
							{{data.row.lteacher_order}}
						</template>
					</el-table-column>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/app_lily/app_lily__tea__list')
			//页码
		...
		methods: {
			funcEdit(row) {
				...
				this.g_cc.func_redirect('/app_lily/tea__edit', {
					id: id,
					callback_url: '/app_lily/tea_list'
				})
			}
			////////////////////
		}
