		...
		<el-main class='x-main'>
			...
			<div class="table-wrap">
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="套餐卡数量">
						<template slot-scope="data">
							<div v-html="data.row.card_count"></div>
						</template>
					</el-table-column>
					<el-table-column label="状态">
						<template slot-scope="data">
							<b style="color:green;" v-if="data.row.lclass_hide==0" v-html="data.row.__hide"></b>
							<div v-if="data.row.lclass_hide==1" v-html="data.row.__hide"></div>
						</template>
					</el-table-column>
					<el-table-column label="排序" width="100">
						<template slot-scope="data">
							<div v-html="data.row.lclass_order"></div>
						</template>
					</el-table-column>
					<el-table-column fixed="right" label="操作">
						<template slot-scope="data">
							<el-button type="primary" size="mini" @click="funcEdit(data.row)">编辑</el-button>
							<router-link class="inlineBlock" :to="`class__card__list?pid=${data.row.id}&callback_url=class__list`">
								<el-button size="mini">查看可使用的套餐卡</el-button>
							</router-link>
						</template>
					</el-table-column>
				</el-table>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/app_lily/app_lily__class__list')
			//页码
		...
		methods: {
			funcEdit(row) {
				...
				this.g_cc.func_redirect('/app_lily/class__edit', {
					id: id,
					callback_url: '/app_lily/class__list'
				})
			}
			////////////////////
		}
