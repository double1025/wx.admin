		...
		<el-main class='x-main'>
			<el-form :inline="true" :model="form_q" size="mini">
				<el-form-item label="套餐卡名称">
					<el-input name="compKw" v-model="form_q.compKw" placeholder="套餐卡名称"></el-input>
				</el-form-item>
				<el-form-item>
				...
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="套餐卡标题">
						<template slot-scope="data">
							<div v-html="data.row.lcitem_title"></div>
						</template>
					</el-table-column>
					...
					<el-table-column label="每次扣课">
						<template slot-scope="data">
							<div v-html="data.row.lczc_use_x_1"></div>
						</template>
					</el-table-column>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/app_lily/app_lily__class__card__list')
			this.g_page.funcSetApiUrlDel('/xadmin/app_lily/app_lily__class__card__list/del')
			//页码
		...
		methods: {
			...
			funcEdit(row) {
				...
				this.g_cc.func_redirect('class__card__edit', {
					id: id,
					pid: this.$route.query.pid,
					callback_url: `class__card__list?pid=${this.$route.query.pid}`
				})
			}
			////////////////////
		}
