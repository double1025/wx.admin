<template>
	<el-container>
		<el-header class='x-header'>
			...
			<el-button size="mini" type="primary" icon="el-icon-edit" @click="g_page.funcRedirectEdit(`app_grade__school__add`)">
				批量导入
			</el-button>
			<el-button size="mini" type="success" icon="el-icon-edit" @click="g_page.funcRedirectEdit(`app_grade__school__edit`)">
				添加
			</el-button>
		...
		<el-main class='x-main'>
			<el-form :inline="true" :model="form_q" size="mini">
				<el-form-item label="学校">
					<el-input name="compKw" v-model="form_q.compKw" placeholder="学校"></el-input>
				</el-form-item>
				<el-form-item>
...
<!--							{{ data.row.abc_str_00 }} {{ data.row.abc_str_01 }} {{ data.row.abc_str_02 }}-->
<!--						</template>-->
<!--					</el-table-column>-->
					<el-table-column label="学校名称">
						<template slot-scope="data">
							<div v-html="data.row.abc_str_03"></div>
						</template>
					</el-table-column>
					<el-table-column label="排序">
						<template slot-scope="data">
							<div v-html="data.row.abc_order"></div>
						</template>
					</el-table-column>
					<el-table-column fixed="right" label="操作">
						<template slot-scope="data">
							<el-button type="primary" size="mini" @click="g_page.funcRedirectEdit('app_grade__school__edit',data.row)">
								编辑
							</el-button>
						</template>
					</el-table-column>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/app_grade/app_grade__school__list')
			this.g_page.funcSetApiUrlDel('/xadmin/app_grade/app_grade__school__list/del')
			//页码
