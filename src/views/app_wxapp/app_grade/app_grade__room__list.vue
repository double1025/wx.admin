<template>
	<el-container>
		<el-header class='x-header'>
			...
			<!--			<ExprotExcel api_url="/xadmin/app_grade/app_grade__user__list/export" :api_data="form_q"></ExprotExcel>-->
			<el-button size="mini" icon="el-icosn-refresh" @click="g_page.funcReload()">刷新</el-button>
		</el-header>
		<el-main class='x-main'>
			<el-form :inline="true" :model="form_q" size="mini">
				<el-form-item label="房间号">
					<el-input v-model="form_q.compKw" placeholder="房间号或授课人名称"></el-input>
				</el-form-item>
				<el-form-item label="openid">
					<el-input v-model="form_q.openid" placeholder="openid"></el-input>
				</el-form-item>
			...
			<div class="table-wrap">
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="观课时间">
						<template slot-scope="data">
							<div v-html="data.row.room_date_begin"></div>
							<div v-html="data.row.room_date_end"></div>
						</template>
					</el-table-column>
					<el-table-column label="有效评价数">
						<template slot-scope="data">
							<div v-html="data.row.rtxt_grade_uv"></div>
						</template>
					</el-table-column>
					<el-table-column label="综合得分">
						<template slot-scope="data">
							<div v-html="data.row.avg_total"></div>
						</template>
					</el-table-column>
					<el-table-column label="创建时间">
						<template slot-scope="data">
							<div v-html="data.row.room_create_date"></div>
						</template>
					</el-table-column>
					<el-table-column fixed="right" label="操作">
						<template slot-scope="data">
							<el-button type="primary" size="mini" @click="g_page.funcRedirectEdit(`app_grade__room_answer__list?pid=${data.row.id}`)">
								评课记录
							</el-button>
						</template>
					</el-table-column>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/app_grade/app_grade__room__list')
			// this.g_page.funcSetApiUrlDel('/xadmin/app_grade/app_grade__room__list/del')
			//页码
