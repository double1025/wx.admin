				...
				<el-tabs type="border-card" v-model="page_tag">
					...
					<el-tab-pane :label="page_info.title" name="1">
						<el-form-item label="说明">
							<a target="_blank" :href="`${page_info.static_file_root}view/app/app_grade/xls/学校导入模板.xlsx`">模板下载</a>
						</el-form-item>
...
<script>
	...
	export default {
		...
		data() {
			let page_data = {
				api_url: '/xadmin/app_grade/app_grade__school__add/import'
			}
			//
		...
		mounted() {
			...
			//
			this.g_page.funcSetApiUrlEdit('/xadmin/app_grade/app_grade__school__add')
			let page = this
			//验证
			this.form_rules = {
				__test: [{
