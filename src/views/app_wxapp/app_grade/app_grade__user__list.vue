			...
			<div class="table-wrap">
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="手机号">
						<template slot-scope="data">
							<div v-html="data.row.winfo_phone"></div>
						</template>
					</el-table-column>
					<el-table-column label="身份">
						<template slot-scope="data">
							<div v-if="data.row.winfo_int_00==1">教师</div>
							<div v-if="data.row.winfo_int_00!=1">游客</div>
						</template>
					</el-table-column>
					<el-table-column label="学校">
						<template slot-scope="data">
							{{ data.row.winfo_province }}{{ data.row.winfo_city }}{{ data.row.winfo_district }}
							{{ data.row.winfo_address }}
						</template>
					</el-table-column>
					<el-table-column label="学科">
						<template slot-scope="data">
							<div v-html="data.row.winfo_str_00"></div>
						</template>
					</el-table-column>
					<el-table-column label="年级">
						<template slot-scope="data">
							<div v-html="data.row.winfo_str_01"></div>
						</template>
					</el-table-column>
					<el-table-column label="企业代码">
						<template slot-scope="data">
							<div v-html="data.row.winfo_str_02"></div>
						</template>
					</el-table-column>
				</el-table>
				<a target="_blank"></a>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/app_grade/app_grade__user__list')
