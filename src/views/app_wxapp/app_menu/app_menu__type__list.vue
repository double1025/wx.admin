<template>
	<el-container>
		<el-header class='x-header'>
			...
			<el-button size="mini" type="success" icon="el-icon-edit" @click="g_page.funcRedirectEdit(`app_menu__type__edit`)">
				添加
			</el-button>
			...
			<div class="table-wrap">
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column fixed="right" label="操作">
						<template slot-scope="data">
							<el-button type="primary" size="mini" @click="g_page.funcRedirectEdit('app_menu__type__edit',data.row)">
								编辑
							</el-button>
							<el-button size="mini" @click="g_page.funcRedirectEdit(`app_menu__type2__list?pid=${data.row.id}`)">
								烹饪方式配置
							</el-button>
						</template>
					</el-table-column>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/app_menu/app_menu__type__list')
			this.g_page.funcSetApiUrlDel('/xadmin/app_menu/app_menu__type__list/del')
			//页码
