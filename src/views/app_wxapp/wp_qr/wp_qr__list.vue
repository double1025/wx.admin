			...
			<div class="table-wrap">
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="名称">
						<template slot-scope="data">
							<div v-html="data.row.wxqr_name"></div>
						</template>
					</el-table-column>
					...
					<el-table-column label="小程序码">
						<template slot-scope="data">
							<CompImg :img_root="page_info.static_file_root" :img_val="data.row.wxqr_info"></CompImg>
						</template>
					</el-table-column>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/wp_qr/wp_qr__list')
			this.g_page.funcSetApiUrlDel('/xadmin/wp_qr/wp_qr__list/del')
			//页码
		...
		methods: {
			funcEdit(row) {
				...
				this.g_cc.func_redirect('wp_qr__edit', {
					id: id,
					callback_url: 'wp_qr__list'
				})
			}
			////////////////////
		}
