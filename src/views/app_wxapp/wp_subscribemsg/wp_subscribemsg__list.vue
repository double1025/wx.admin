<template>
	<el-container>
		<el-header class='x-header'>
			...
			<el-button size="mini" type="success" icon="el-icon-edit" @click="funcEdit({id:''})">添加</el-button>
				...
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="名称">
						<template slot-scope="data">
							<div v-html="data.row.wxsg_name"></div>
						</template>
					</el-table-column>
					<el-table-column label="模板ID">
						<template slot-scope="data">
							{{data.row.wxsg_template_id}}
						</template>
					</el-table-column>
		...
		<!---->
		<!-- 编辑控件 -->
		<SubscribemsgEdit ref="item_edit" :uix="page_info.uix" :funcSaveAfter="funcEditItem_success"></SubscribemsgEdit>
		<!---->
		<!---->
		<!---->
		<!---->
...
<script>
	...
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/wp_subscribemsg/wp_subscribemsg__list')
			this.g_page.funcSetApiUrlDel('/xadmin/wp_subscribemsg/wp_subscribemsg__list/del')
			//页码
		...
		methods: {
			funcEdit(row) {
				this.$refs['item_edit'].funcEdit(row.id)
			},
