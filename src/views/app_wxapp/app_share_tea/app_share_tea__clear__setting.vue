			...
			<el-form ref="form" :rules="form_rules" :model="form" size="mini" label-width="15%">
				<el-tabs type="border-card" v-model="page_tag">
					...
					<el-tab-pane :label="page_info.title" name="1">
						<el-form-item label="通知保洁员-模板消息" prop="abc_str_01">
							<el-input class="x-input-60" v-model="form.abc_str_01"></el-input>
							<div class="x-alert">
								<span>有清洁任务会通过公众号通知保洁员</span><br/>
								<span>{$msg}：通知内容</span>
								<span>{$fun}：积分</span>
								<span>{$money}：余额</span>
							</div>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			//
			this.g_page.funcSetApiUrlEdit('/xadmin/app_share_tea/app_share_tea__clear__setting')
			this.g_page.funcSetApiUrlSave('/xadmin/app_share_tea/app_share_tea__clear__setting/save')
			let page = this
			//验证
			this.form_rules = {
				__test: [{
