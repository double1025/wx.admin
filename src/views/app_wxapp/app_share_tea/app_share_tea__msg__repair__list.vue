			...
			<div class="table-wrap">
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="状态">
						<template slot-scope="data">
							<div v-if="data.row.log_int_00==1">已完成</div>
							<el-button v-if="data.row.log_int_00==0" type="text" size="mini" @click="funcRead(data.row.id)">
								进行审核
							</el-button>
						</template>
					</el-table-column>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/app_share_tea/app_share_tea__msg__repair__list')
			this.g_page.funcSetApiUrlDel('/xadmin/app_share_tea/app_share_tea__msg__repair__list/del')
			//页码
			...
			funcRead(id) {
				...
				this.g_cc.func_confirm('确定？', () => {
					this.g_cc.func_axios({
						url: '/xadmin/app_share_tea/app_share_tea__msg__repair__list/funcRead',
						data: {
							id: id
						},
						success: (res) => {
