<template>
	<el-container>
		<el-header class='x-header'>
			...
			<el-button size="mini" type="success" icon="el-icon-edit" @click="g_page.funcRedirectEdit(`app_share_tea__clear__edit`)">
				添加
			</el-button>
				...
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="编号">
						<template slot-scope="data">
							<div v-html="data.row.tcu_id"></div>
						</template>
					</el-table-column>
					...
					<el-table-column label="姓名">
						<template slot-scope="data">
							<div v-html="data.row.tcu_name"></div>
					...
					<el-table-column label="手机号">
						<template slot-scope="data">
							<div v-html="data.row.tcu_phone"></div>
						</template>
					</el-table-column>
					<el-table-column label="入职时间">
						<template slot-scope="data">
							<div v-html="data.row.entry_date"></div>
						</template>
					</el-table-column>
					<el-table-column label="工作时间">
						<template slot-scope="data">
							{{ data.row.tcu_week_time_begin }}-{{data.row.tcu_week_time_end}}
						</template>
					</el-table-column>
					<el-table-column label="累计完成">
						<template slot-scope="data">
							<div v-html="data.row.tcu_get_count"></div>
						</template>
					</el-table-column>
					<el-table-column label="状态">
						<template slot-scope="data">
							<div v-if="data.row.tcu_enable==1">生效</div>
							<div style="color:red;" v-if="data.row.tcu_enable!=1">停用</div>
						</template>
					</el-table-column>
					<el-table-column fixed="right" label="操作">
						<template slot-scope="data">
							<el-button type="primary" size="mini" @click="g_page.funcRedirectEdit('app_share_tea__clear__edit',data.row)">
								编辑
							</el-button>
						</template>
					</el-table-column>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/app_share_tea/app_share_tea__clear__list')
			this.g_page.funcSetApiUrlDel('/xadmin/app_share_tea/app_share_tea__clear__list/del')
			//页码
