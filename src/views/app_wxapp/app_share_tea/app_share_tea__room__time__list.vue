<template>
	<el-container>
		<el-header class='x-header'>
			...
			<el-button size="mini" type="success" icon="el-icon-edit" @click="g_page.funcRedirectEdit(`app_share_tea__room__time__edit?pid=${page_info.entity_room.troom_id}`)">
				添加
			</el-button>
		...
		<el-main class='x-main'>
			<el-form :inline="true" :model="form_q" size="mini">
				...
				<div v-if="page_info.entity_room" class="x-alert">
					<span>茶室价格：{{ page_info.entity_room.troom_time_price }}元/半小时</span>
				</div>
			</el-form>
				...
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="编号">
						<template slot-scope="data">
							<div v-html="data.row.trt_id"></div>
						</template>
					</el-table-column>
					<el-table-column label="时间点">
						<template slot-scope="data">
							{{ data.row.trt_day }} {{ data.row.trt_time }}
						</template>
					</el-table-column>
					<el-table-column label="价格">
						<template slot-scope="data">
							{{data.row.trt_price}}元/半小时
						</template>
					</el-table-column>
				</el-table>
				<a target="_blank"></a>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/app_share_tea/app_share_tea__room__time__list')
			this.g_page.funcSetApiUrlDel('/xadmin/app_share_tea/app_share_tea__room__time__list/del')
			//页码
