				...
				<el-table stripe ref="multipleTable" tooltip-effect="dark" @selection-change="g_page.funcTableSelectionChange"
					...
					<el-table-column label="报错信息">
						<template slot-scope="data">
							<div style="color:red;" v-html="data.row.log_str_00"></div>
						</template>
					</el-table-column>
					<el-table-column label="报错详情">
						<template slot-scope="data">
							<el-input type="textarea" :rows="5" v-model="data.row.log_txt_00"></el-input>
						</template>
					</el-table-column>
					<el-table-column label="时间" width="180">
						<template slot-scope="data">
							{{data.row.log_create_date}}
						</template>
					</el-table-column>
				</el-table>
				<a target="_blank"></a>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/sys_log/sys_log__list')
			//页码
