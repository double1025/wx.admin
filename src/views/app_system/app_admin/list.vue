			...
			<div class="table-wrap">
				<el-table stripe ref="multipleTable" tooltip-effect="dark" @selection-change="g_page.funcTableSelectionChange"
					...
					<el-table-column fixed="right" label="操作">
						<template slot-scope="data">
							<el-button v-if="data.row.app_do_reply==1" size="mini" icon="el-icon-s-tools" type="text" plain
									   @click="g_page.funcRedirectEdit(`edit`,data.row)">
							</el-button>
							<el-button size="mini" type="text" @click="g_page.funcRedirectEdit(`app_admin__set__list?pid=${data.row.id}`)">
								属性模板
							</el-button>
							<el-button size="mini" type="text" @click="g_page.funcRedirectEdit(`/view_admin/view_admin__page__list?app_id=${data.row.id}`)">
								页面
							</el-button>
						</template>
					</el-table-column>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/app_admin/app_admin__list')
