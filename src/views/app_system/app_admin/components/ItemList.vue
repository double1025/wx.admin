	...
	<div>
		<div id="list_item" class="thumb-container-x">
			...
			<div v-for="(item, i) in list_data" :key="item.id" class="thumb-item">
				<span class="thumb-item-txt">{{item.sitem_type}}</span>
				<span class="thumb-item-txt">名称：{{item.sitem_label}}</span>
				<span class="thumb-item-txt">key：{{item.sitem_key}}</span>
				<div class="control">
		...
		<!---->
		<!-- 添加 -->
		<div class="thumb-item-add" @click="funcEditItem('')">
...
<script>
	...
	import ItemEdit from '@/views/app_system/app_admin/components/ItemEdit'
	...
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/app_admin/app_admin__set_item__list')
			this.g_page.funcSetApiUrlDel('/xadmin/app_admin/app_admin__set_item__list/del')
			//页码
		...
		methods: {
			...
			funcItemSetOrder(list_index) {
				this.g_cc.func_axios({
					url: '/xadmin/app_admin/app_admin__set_item__list/funcItemSetOrder',
					data: {
