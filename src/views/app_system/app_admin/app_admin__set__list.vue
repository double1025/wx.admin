<template>
	<el-container>
		<el-header class='x-header'>
			...
			<el-button v-if="form_q.do_add" size="mini" type="success" icon="el-icon-edit" @click="g_page.funcRedirectEdit(`app_admin__set__edit?pid=${form_q.pid}`)">
				添加
			</el-button>
				...
				<el-table stripe ref="multipleTable" tooltip-effect="dark" @selection-change="g_page.funcTableSelectionChange"
                  ...
                  <el-table-column label="应用">
                    <template slot-scope="data">
                      <div>{{form_q.app_name}}</div>
                      <div>{{data.row.set_app_id}}</div>
                    </template>
                  </el-table-column>
					<el-table-column label="ID">
						<template slot-scope="data">
                          <div>{{data.row.set_id}}</div>
						</template>
					</el-table-column>
					<el-table-column label="名称">
						<template slot-scope="data">
							<div v-html="data.row.set_name"></div>
						</template>
					</el-table-column>
					<el-table-column label="排序">
						<template slot-scope="data">
							<div v-html="data.row.set_order"></div>
						</template>
					</el-table-column>
					<el-table-column fixed="right" label="操作">
						<template slot-scope="data">
							<el-button type="primary" size="mini" @click="g_page.funcRedirectEdit('app_admin__set__edit',data.row)">
								编辑
							</el-button>
						</template>
					</el-table-column>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/app_admin/app_admin__set__list')
			this.g_page.funcSetApiUrlSave('/xadmin/app_admin/app_admin__set__list/save')
			//页码
