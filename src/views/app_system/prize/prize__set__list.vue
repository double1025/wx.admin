			...
			<div class="table-wrap">
				<el-table stripe ref="multipleTable" tooltip-effect="dark" @selection-change="g_page.funcTableSelectionChange"
					...
					<el-table-column label="奖品图">
						<template slot-scope="data">
							<CompImg :show_size="true" :img_root="page_info.static_file_root" :img_val="data.row.pset_pic"></CompImg>
						</template>
					</el-table-column>
					<el-table-column label="中奖率">
						<template slot-scope="data">
							<div>{{ data.row.pschou_win_rate }}</div>
						</template>
					</el-table-column>
					<el-table-column label="余量/总量">
						<template slot-scope="data">
							<div>{{ data.row.stock_count_left }} / {{ data.row.stock_count_left +
								data.row.stock_count_use }}
							</div>
						</template>
					</el-table-column>
