				...
				<el-tabs type="border-card" v-model="page_tag">
					...
					<el-tab-pane :label="page_info.title" name="1">
						...
						<el-form-item label="文件内容">
							<el-input type="textarea" :rows="20" v-model="form.txt"></el-input>
						</el-form-item>
					</el-tab-pane>
					<!---->
					<!---->
...
<script>
	export default {
		...
		mounted() {
			...
			//
			this.g_page.funcSetApiUrlEdit('/xadmin/file/file__edit')
			this.g_page.funcSetApiUrlSave('/xadmin/file/file__edit/save')
			let page = this
			//验证
			this.form_rules = {
				__test: [{
		...
		methods: {
			funcShowDialogAfter() {
				...
				let key = this.page_info.encode_key
				let vi = this.page_info.encode_vi
				let x = this.g_cc.funcAesEn('123', key, vi)
				console.log(x)
			},
			//保存之前
			funcSaveBefore(data) {
				...
				let key = this.page_info.encode_key
				let vi = this.page_info.encode_vi
				data.txt = this.g_cc.funcAesEn(data.txt, key, vi)
