<template>
	<el-container>
		<el-header class='x-header'>
			...
			<!--			<el-button size="mini" icon="el-icon-refresh" @click="test()">test</el-button>-->
		</el-header>
		<el-main class='x-main'>
			<el-form :inline="true" :model="form_q" size="mini">
				...
				<el-form-item label="手机号">
					<el-input name="compKw" v-model="form_q.compPhone" placeholder="手机号"></el-input>
				</el-form-item>
				<el-form-item>
			...
			<div class="table-wrap">
				<el-table stripe ref="multipleTable" tooltip-effect="dark" @selection-change="g_page.funcTableSelectionChange"
					...
					<el-table-column label="头像">
						<template slot-scope="data">
							<CompImg :img_root="page_info.static_file_root" :img_val="data.row.wxuser_headimgurl"></CompImg>
						</template>
					</el-table-column>
					<el-table-column label="积分">
						<template slot-scope="data">
							<div>{{data.row.wxuser_fun}}</div>
						</template>
					</el-table-column>
					<el-table-column label="余额">
						<template slot-scope="data">
							<div>{{data.row.wxuser_money}}</div>
						</template>
					</el-table-column>
					<el-table-column fixed="right" label="操作">
						<template slot-scope="data">
							<el-button type="primary" size="mini" @click="g_page.funcRedirectEdit('user__edit',data.row)">
								详情
							</el-button>
						</template>
					</el-table-column>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/user/user__list')
		...
		methods: {
			test() {
				console.log('test', this.page_index)
				// this.page_index = 2
			},
			funcSaveBefore(data) {
