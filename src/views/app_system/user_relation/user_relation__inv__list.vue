<template>
	<el-container>
		<el-header class='x-header'>
			...
			<el-button size="mini" type="success" icon="el-icon-edit" @click="g_page.funcRedirectEdit(`user_relation__item__edit`)">
				添加
			</el-button>
		...
		<el-main class='x-main'>
			<el-form :inline="true" :model="form_q" size="mini">
				<el-form-item label="主人">
					<el-input name="compKw" v-model="form_q.compKw" placeholder="openid"></el-input>
				</el-form-item>
			...
			<div class="table-wrap">
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="主人">
						<template slot-scope="data">
							<div v-html="data.row.master_wxuser_nickName"></div>
							<div v-html="data.row.master_wxuser_openid"></div>
						</template>
					</el-table-column>
					<el-table-column label="好友">
						<template slot-scope="data">
							<div v-html="data.row.friend_wxuser_nickName"></div>
							<div v-html="data.row.friend_wxuser_openid"></div>
						</template>
					</el-table-column>
					<el-table-column label="绑定时间">
						<template slot-scope="data">
							{{ data.row.wur_create_date }}
						</template>
					</el-table-column>
				</el-table>
				<a target="_blank"></a>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/user_relation/user_relation__inv__list')
			this.g_page.funcSetApiUrlDel('/xadmin/user_relation/user_relation__inv__list/del')
			//页码
