			...
			<div class="table-wrap">
				<el-table stripe ref="multipleTable" tooltip-effect="dark"
					...
					<el-table-column label="背景图">
						<template slot-scope="data">
							<CompImg :img_root="page_info.static_file_root" :show_size="true"
									 :img_val="data.row.abc_str_00">
							</CompImg>
						</template>
					</el-table-column>
					<el-table-column label="隐藏">
						<template slot-scope="data">
							<div v-if="data.row.abc_hide==1">隐藏</div>
							<div v-if="data.row.abc_hide==0">显示</div>
						</template>
					</el-table-column>
					...
					<el-table-column fixed="right" label="操作">
						<template slot-scope="data">
							<el-button type="primary" size="mini" @click="g_page.funcRedirectEdit('user_relation__item__edit',data.row)">
								编辑
							</el-button>
						</template>
					</el-table-column>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/user_relation/user_relation__item__list')
			this.g_page.funcSetApiUrlDel('/xadmin/user_relation/user_relation__item__list/del')
			//页码
