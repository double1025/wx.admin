				...
				<el-table stripe ref="multipleTable" tooltip-effect="dark" @selection-change="g_page.funcTableSelectionChange"
					...
					<el-table-column label="账号">
						<template slot-scope="data">
							<div>{{ super_uid
								}}@{{data.row.acc_uid}}<span v-if="data.row.acc_lock==1" style="color: red;">[停用]</span>
							</div>
						</template>
					</el-table-column>
					...
					<el-table-column fixed="right" label="操作">
						<template slot-scope="data">
							<el-button type="primary" size="mini" @click="g_page.funcShowDialog(data.row)">编辑
							</el-button>
...
<script>
	export default {
		...
		mounted() {
			...
			// 应用ID
			this.g_page.funcSetApiUrlList('/xadmin/account/account_list')
			this.g_page.funcSetApiUrlSave('/xadmin/account/account_list/save')
			this.g_page.funcSetApiUrlDel('/xadmin/account/account_list/del')
			//页码
			...
			funcResetPwd(row) {
				...
				page.g_cc.func_confirm('重置密码为123456，确认吗', function() {
					page.g_cc.func_axios({
						url: '/xadmin/account/account_list/reset_pwd',
						data: {
							ids: row.id
						},
